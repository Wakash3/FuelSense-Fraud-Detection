...
const { getAlerts, acknowledgeAlert, checkVarianceAlert, checkHighWaterAlert, checkLowStockAlert } = require('./alerts');
...
// ── GET /api/alerts ───────────────────────────────────────────────────────
// Returns alerts filtered by status (open, acknowledged, resolved, all)
...
// Returns count of open alerts by severity — used by dashboard banner
...
// ── GET /api/pump-vs-dip ──────────────────────────────────────────────────
// Returns all closed shifts with pump vs dip comparison
...
setTimeout(async () => {
    try {
        ...
        const { checkHighWaterAlert, checkLowStockAlert } = require('./alerts');
        ...
        async function pollCycle() {
            ...
            for (const reading of readings) {
                try {
                    ...
                    // Run alert checks on every reading
                    ...
                    if (!state) {
                        tankState[t.id] = { lastInnageMm: reading.innageMm, stableCycles: 0, risingCycles: 0, deliveryId: null, deliveryStatus: 'none' };
                        continue;
                    }
